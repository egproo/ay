		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateUploadForm())) {
			if ((isset( $this->request->files['upload'] )) && (is_uploaded_file($this->request->files['upload']['tmp_name']))) {
				...
				if ($this->model_extension_export_import->upload($file,$this->request->post['incremental'])==true) {
					...
					if($this->session->data['export_accounts_success'] == '1'){
					$this->response->redirect($this->url->link('accounts/chartaccount', 'user_token=' . $this->session->data['user_token'], $this->ssl));
					}else{
