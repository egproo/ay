...
class ControllerSupplierSupplier extends Controller {
    ...
    private function generateAccountCode() {
        $base_code = 32411; // تعديل الكود الأساسي حسب شجرة الحسابات الخاصة بك
        $last_account_code = $this->db->query("SELECT MAX(account_code) AS max_code FROM " . DB_PREFIX . "supplier WHERE account_code LIKE '32411%'")->row['max_code'];
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$supplier_id = $this->model_supplier_supplier->addsupplier($this->request->post);

            // إضافة حساب إلى شجرة الحسابات
            $account_code = $this->generateAccountCode();
            $account_data = array(
                'account_code' => $account_code,
                'parent_id' => 32411, // نفترض أن حساب العملاء يقع تحت الكود 32411
                'status' => 1, // حالة الحساب
                'account_description' => array(
                    1 => array('name' => $this->request->post['firstname'])
                )
            );
            $this->addAccount($account_data);

            $this->db->query("UPDATE " . DB_PREFIX . "supplier SET account_code = '" . (int)$account_code . "' WHERE supplier_id = '" . (int)$supplier_id . "'");
	...
	public function unlock() {
		...
		if (isset($this->request->get['email']) && $this->validateUnlock()) {
			$this->model_supplier_supplier->deleteLoginAttempts($this->request->get['email']);
	...
	protected function getList() {
		...
		$data['sort_email'] = $this->url->link('supplier/supplier', 'user_token=' . $this->session->data['user_token'] . '&sort=c.email' . $url, true);
		$data['sort_telephone'] = $this->url->link('supplier/supplier', 'user_token=' . $this->session->data['user_token'] . '&sort=c.telephone' . $url, true);


		$data['sort_supplier_group'] = $this->url->link('supplier/supplier', 'user_token=' . $this->session->data['user_token'] . '&sort=supplier_group' . $url, true);
		$data['sort_status'] = $this->url->link('supplier/supplier', 'user_token=' . $this->session->data['user_token'] . '&sort=c.status' . $url, true);
		$data['sort_ip'] = $this->url->link('supplier/supplier', 'user_token=' . $this->session->data['user_token'] . '&sort=c.ip' . $url, true);
		$data['sort_date_added'] = $this->url->link('supplier/supplier', 'user_token=' . $this->session->data['user_token'] . '&sort=c.date_added' . $url, true);
		...
		$data['filter_supplier_group_id'] = $filter_supplier_group_id;
		...
		$data['supplier_groups'] = $this->model_customer_customer_group->getCustomerGroups();
	...
	protected function getForm() {
		...
		if (isset($this->request->post['supplier_group_id'])) {
			...
			$data['supplier_group_id'] = $supplier_info['supplier_group_id'];
		} else {
			$data['supplier_group_id'] = $this->config->get('config_supplier_group_id');
		}
		...
		if (isset($this->request->post['custom_field'])) {
			...
			$data['account_custom_field'] = json_decode($supplier_info['custom_field'], true);
		} else {
		...
		if (isset($this->request->post['address'])) {
			...
			$data['addresses'] = $this->model_supplier_supplier->getAddresses($this->request->get['supplier_id']);
		} else {
		...
		if (isset($this->request->post['newsletter'])) {
			...
			$data['newsletter'] = $supplier_info['newsletter'];
		} else {
			$data['newsletter'] = '';
		}
		...
		if (isset($this->request->post['address_id'])) {
			...
			$data['address_id'] = $supplier_info['address_id'];
		} else {
			$data['address_id'] = '';
		}
		...
		// Affliate
		if (isset($this->request->get['supplier_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$affiliate_info = $this->model_supplier_supplier->getAffiliate($this->request->get['supplier_id']);
		}
	...
	protected function validateForm() {
		...
		$supplier_info = $this->model_supplier_supplier->getsupplierByEmailOrPhone($this->request->post['email']);
		$supplier_info2 = $this->model_supplier_supplier->getsupplierByEmailOrPhone($this->request->post['telephone']);



		if (!isset($this->request->get['supplier_id'])) {
			if ($supplier_info || $supplier_info2) {
			...
			if (($supplier_info2) && ($this->request->get['supplier_id'] != $supplier_info2['supplier_id'])) {
				$this->error['warning'] = $this->language->get('error_exists');
			}			
		}
		...
		if ($this->request->post['affiliate']) {
			...
			$affiliate_info = $this->model_supplier_supplier->getAffiliateByTracking($this->request->post['tracking']);
			...
			if (!isset($this->request->get['supplier_id'])) {
				...
				if ($affiliate_info && ($this->request->get['supplier_id'] != $affiliate_info['supplier_id'])) {
					$this->error['tracking'] = $this->language->get('error_tracking_exists');
				}
			}
	...
	public function login() {
		...
		if ($supplier_info) {
			...
			$this->model_supplier_supplier->editToken($supplier_id, $token);
	...
	public function ip() {
		...
		$results = $this->model_supplier_supplier->getIps($this->request->get['supplier_id'], ($page - 1) * $limit, $limit);

		foreach ($results as $result) {
			$data['ips'][] = array(
				'ip'         => $result['ip'],
				'total'      => $this->model_supplier_supplier->getTotalsuppliersByIp($result['ip']),
				'date_added' => date('d/m/y', strtotime($result['date_added'])),
				'filter_ip'  => $this->url->link('supplier/supplier', 'user_token=' . $this->session->data['user_token'] . '&filter_ip=' . $result['ip'], true)
			);
		}

		$ip_total = $this->model_supplier_supplier->getTotalIps($this->request->get['supplier_id']);

		$pagination = new Pagination();
		$pagination->total = $ip_total;
		$pagination->page = $page;
		$pagination->limit = $limit;
		$pagination->url = $this->url->link('supplier/supplier/ip', 'user_token=' . $this->session->data['user_token'] . '&supplier_id=' . $this->request->get['supplier_id'] . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('supplier/supplier_ip', $data));
	}
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_email'])) {
			...
			foreach ($results as $result) {
				$json[] = array(
					...
					'address'           => $this->model_supplier_supplier->getAddresses($result['supplier_id'])
				);
			}
		}
	...
	public function customfield() {
		...
		$this->load->model('supplier/custom_field');

		// supplier Group
		if (isset($this->request->get['supplier_group_id'])) {
			$supplier_group_id = $this->request->get['supplier_group_id'];
		} else {
			$supplier_group_id = $this->config->get('config_supplier_group_id');
		}

		$custom_fields = $this->model_supplier_custom_field->getCustomFields(array('filter_supplier_group_id' => $supplier_group_id));
	...
	public function address() {
		...
		if (!empty($this->request->get['address_id'])) {
			...
			$json = $this->model_supplier_supplier->getAddress($this->request->get['address_id']);
		}
