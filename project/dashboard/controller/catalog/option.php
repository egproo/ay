...
public function autocomplete() {
    ...
    if (isset($this->request->get['filter_name'])) {
        ...
        foreach ($options as $option) {
            ...
            // الحصول على اسم الوحدة إذا كان متاحًا
            $unit_name = '';
            if (isset($option['unit_id']) && $option['unit_id']) {
                $unit_info = $this->model_catalog_product->getUnit($option['unit_id']);
                if ($unit_info) {
            ...
            $json[] = array(
                ...
                'category'     => $type,
                'type'         => $option['type'],
                'unit_id'      => isset($option['unit_id']) ? $option['unit_id'] : '',
                'unit_name'    => $unit_name,
	...
	public function autocompleteq(): void {
		$json = [];
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($options as $option) {
				...
				foreach($option_value_data as $optvalue){
				$json[] = [
					'option_id'    => $optvalue['option_value_id'],
					'name'      => strip_tags(html_entity_decode($option['name'] . ' &gt; ' . $optvalue['name'], ENT_QUOTES, 'UTF-8')),
					'category'     => $type,
					'type'         => $option['type'],
					'option_value' => $option_value_data
				];				    
