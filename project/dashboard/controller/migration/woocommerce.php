...
class Woocommerce extends \Opencart\System\Engine\Controller {
    public function index() {
        $this->load->language('migration/migration');

        $this->document->setTitle($this->language->get('text_woocommerce_migration'));
        ...
        $data['breadcrumbs'][] = [
            ...
            'href' => $this->url->link('migration/woocommerce', 'user_token=' . $this->session->data['user_token'])
        ];

        $data['heading_title'] = $this->language->get('text_woocommerce_migration');
        ...
        // Load the template
        $this->response->setOutput($this->load->view('migration/woocommerce', $data));
    }
    ...
    public function import() {
        ...
        if (!$this->user->hasPermission('modify', 'migration/woocommerce')) {
            ...
            try {
                ...
                if (empty($this->request->post['consumer_key']) || empty($this->request->post['consumer_secret'])) {
                    $json['error'] = $this->language->get('error_credentials');
                } else {
                    // Process the import
                    $import_data = $this->model_migration_migration->processWooCommerceImport([
                ...
                $this->log->write('WooCommerce import error: ' . $e->getMessage());
            }
        }
