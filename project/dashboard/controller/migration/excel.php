...
class Excel extends \Opencart\System\Engine\Controller {
    public function index() {
        $this->load->language('migration/migration');

        $this->document->setTitle($this->language->get('text_excel_migration'));
        ...
        $data['breadcrumbs'][] = [
            ...
            'href' => $this->url->link('migration/excel', 'user_token=' . $this->session->data['user_token'])
        ];

        $data['heading_title'] = $this->language->get('text_excel_migration');
        ...
        // Load the template
        $this->response->setOutput($this->load->view('migration/excel', $data));
    }
    ...
    public function import() {
        ...
        if (!$this->user->hasPermission('modify', 'migration/excel')) {
            ...
            try {
                if (!isset($this->request->files['file'])) {
                    ...
                    if (!in_array($file_info['extension'], ['xlsx', 'xls', 'csv'])) {
                        $json['error'] = $this->language->get('error_file_type');
                    } else {
                        // Process the import file
                        $import_data = $this->model_migration_migration->processExcelImport($file);
                ...
                $this->log->write('Excel import error: ' . $e->getMessage());
            }
        }
