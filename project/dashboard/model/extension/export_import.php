...
protected function uploadAccounts(&$reader, $incremental, &$available_account_ids=array()) {
    $data = $reader->getSheetByName('Accounts');
    if ($data == null) {
        return;
        $this->session->data['export_accounts_success'] = 0;
    }
	$this->session->data['export_accounts_success'] = 1;
    $languages = $this->getLanguages(); // Assuming you have a method to fetch installed languages
    $first_row = array();
    $i = 0;
    $k = $data->getHighestRow();
    ...
    for ($i = 0; $i < $k; $i++) {
        ...
        $j = 1;
        $account_id = trim($this->getCell($data, $i, $j++));
        if ($account_id == "") {
            continue;
        }
        $account_code = trim($this->getCell($data, $i, $j++));
        $account_type = trim($this->getCell($data, $i, $j++));
        
        $parent_id = trim($this->getCell($data, $i, $j++));
        $names = array();
        while ($this->startsWith($first_row[$j-1], "name(")) {
            $language_code = substr($first_row[$j-1], 5, -1);
        ...
        $available_account_ids[$account_id] = $account_id;
    }
...
public function updateAccount($account) {
    ...
    $existsQuery = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "accounts WHERE account_id = '" . (int)$account['account_id'] . "'";
    $existsResult = $this->db->query($existsQuery);

    if ($existsResult->row['total'] == 0) {
        // Insert new account if it does not exist
        $insertSql = "INSERT INTO " . DB_PREFIX . "accounts (account_id, account_code,account_type, parent_id, status) VALUES ('" . (int)$account['account_id'] . "', '" . $this->db->escape($account['account_code']) . "', '" . (int)$account['parent_id'] . "', '" . (int)$account['status'] . "')";
        $this->db->query($insertSql);
    } else {
        // Update existing account
        $updateSql = "UPDATE " . DB_PREFIX . "accounts SET " .
    ...
    // Update or insert names for all languages
    foreach ($this->getLanguages() as $language) {
        $language_code = $language['code'];
        $language_id = $language['language_id'];

        $name = isset($account['names'][$language_code]) ? $account['names'][$language_code] : null;
        if ($name !== null) {
            $sql = "SELECT COUNT(*) as total FROM " . DB_PREFIX . "account_description " .
                   "WHERE account_id = '" . (int)$account['account_id'] . "' AND language_id = '" . (int)$language_id . "'";
            $result = $this->db->query($sql);
            
            if ($result->row['total'] > 0) {
                // Update existing description
                $sql = "UPDATE " . DB_PREFIX . "account_description SET " .
                       "name = '" . $this->db->escape($name) . "' " .
                       "WHERE account_id = '" . (int)$account['account_id'] . "' AND language_id = '" . (int)$language_id . "'";
            } else {
                // Insert new description if it doesn't exist
                $sql = "INSERT INTO " . DB_PREFIX . "account_description (account_id, language_id, name) VALUES (" .
                       "'" . (int)$account['account_id'] . "', '" . (int)$language_id . "', '" . $this->db->escape($name) . "')";
            }
            $this->db->query($sql);
        }
    }
...
public function insertAccount($account) {
    $sql = "INSERT INTO " . DB_PREFIX . "accounts (account_id,parent_id,account_code,account_type, status) VALUES ('" . (int)$account['account_id'] . "', '" . $this->db->escape($account['account_code']) . "', '" . $this->db->escape($account['account_type']) . "',parent_id = '" . (int)$this->db->escape($account['parent_id']) . "', '" . (int)$account['status'] . "')";
    $this->db->query($sql);
    // Insert names for all languages
    foreach ($account['names'] as $language_code => $name) {
        $language_id = $this->getLanguageIdByCode($language_code);
        $sql = "INSERT INTO " . DB_PREFIX . "account_description (account_id, language_id, name) VALUES ('" . (int)$account['account_id'] . "', '" . (int)$language_id . "', '" . $this->db->escape($name) . "')";
        $this->db->query($sql);
    }
}
...
public function deleteAccount($account_id) {
    $sql = "DELETE FROM " . DB_PREFIX . "accounts WHERE account_id = '" . (int)$account_id . "'";
    $this->db->query($sql);
    $sql = "DELETE FROM " . DB_PREFIX . "account_description WHERE account_id = '" . (int)$account_id . "'";
    $this->db->query($sql);
}
...
//		$col -= 1; // we use 1-based, PHPExcel uses 0-based column index, PhpSpreadsheet now uses 1-based column index
	...
	protected function validateAccounts( &$reader ) {
		$data = $reader->getSheetByName( 'Accounts' );
		if ($data==null) {
			return true;
		}
	...
	$expected_heading = array
				( "account_id","account_code","account_type", "parent_id", "name", "status" );
	...
	protected function validateProductIdColumns( &$reader ) {
		...
		$worksheets = array( 'AdditionalImages','Units','Inventory','Pricing','QuantityDiscounts','Recommendations','Bundles','Barcodes', 'Rewards', 'ProductOptions', 'ProductOptionValues', 'ProductAttributes', 'ProductFilters', 'ProductSEOKeywords' );
	...
	protected function validateCustomerGroupColumns( &$reader ) {
		...
		$worksheets = array('Rewards', 'Customers' );
		$ok = true;
	...
	public function upload( $filename, $incremental=false ) {
		...
		try {
			...
			$this->uploadAccounts( $reader, $incremental, $available_account_ids );
			...
			$this->uploadProductUnits( $reader, $incremental, $available_product_ids );
			$this->uploadInventory( $reader, $incremental, $available_product_ids );
			$this->uploadPricing( $reader, $incremental, $available_product_ids );
			$this->uploadQuantityDiscounts( $reader, $incremental, $available_product_ids );
			$this->uploadRecommendations( $reader, $incremental, $available_product_ids );
			$this->uploadBundles( $reader, $incremental, $available_product_ids );
			$this->uploadBarcodes( $reader, $incremental, $available_product_ids );
...
protected function populateAccountsWorksheet(&$worksheet, &$languages, $box_format, $text_format) {
    $this->load->model('accounts/chartaccount');
    $accounts = $this->model_accounts_chartaccount->getAllAccounts();

    // Set column widths
    $j = 1;
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('account_id'), 10) + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('account_code'), 20) + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('account_type'), 20) + 1);
    foreach ($languages as $language) {
    ...
    $data[$j++] = 'status';  // Add status to the header
    $this->setCellRowx($worksheet, $i, $data, $box_format);
    ...
    foreach ($accounts as $account) {
        $j = 1;
        $data = [];
        $data[$j++] = $account['account_id'];
        $data[$j++] = $account['account_code'];
        $data[$j++] = $account['account_type'];
        $data[$j++] = $account['parent_id'];
        foreach ($languages as $language) {
            if (isset($account['names'][$language['language_id']])) {
                $data[$j++] = $account['names'][$language['language_id']];
            } else {
                $data[$j++] = ''; // Or any other default value
            }
        }
        $data[$j++] = $account['status'];  // Add status to each row
        $this->setCellRowx($worksheet, $i++, $data);
    }
}

protected function setCellRowx($worksheet, $row, $data, $format = null) {
    $col = 1;
    foreach ($data as $value) {
        $cell = $worksheet->getCellByColumnAndRow($col++, $row);
        $cell->setValue($value);
        if ($format) {
            $cell->getStyle()->applyFromArray($format);
        }
    }
}
		...
		try {
			...
			switch ($export_type) {
				...
				case 'accounts':
					...
					$workbook->setActiveSheetIndex($worksheet_index++);
					$worksheet = $workbook->getActiveSheet();
					$worksheet->setTitle( 'Accounts' );
					$this->populateAccountsWorksheet( $worksheet, $languages, $box_format, $text_format, $offset, $rows, $min_id, $max_id );
					$worksheet->freezePaneByColumnAndRow( 2, 2 );
				...
				case 'p':
					...
					$this->populateProductUnitsWorksheet( $worksheet, $box_format, $text_format, $min_id, $max_id );
					...
					$workbook->setActiveSheetIndex($worksheet_index++);
					$worksheet = $workbook->getActiveSheet();
					$worksheet->setTitle( 'Inventory' );
					$this->populateInventoryWorksheet( $worksheet, $price_format, $box_format, $text_format, $min_id, $max_id );
					...
					$workbook->setActiveSheetIndex($worksheet_index++);
					$worksheet = $workbook->getActiveSheet();
					$worksheet->setTitle( 'Pricing' );
					$this->populatePricingWorksheet( $worksheet, $language_id, $price_format,$box_format, $text_format, $min_id, $max_id );
					$worksheet->freezePaneByColumnAndRow( 2, 2 );
					// creating the Rewards worksheet
					$workbook->createSheet();
					$workbook->setActiveSheetIndex($worksheet_index++);
					...
					$this->populateQuantityDiscountsWorksheet( $worksheet, $language_id, $price_format, $box_format, $text_format, $min_id, $max_id );
					$worksheet->freezePaneByColumnAndRow( 2, 2 );
					// creating the Rewards worksheet
					$workbook->createSheet();
					$workbook->setActiveSheetIndex($worksheet_index++);
					$worksheet = $workbook->getActiveSheet();
					$worksheet->setTitle( 'Recommendations' );
					$this->populateRecommendationsWorksheet( $worksheet, $language_id, $price_format, $box_format, $text_format, $min_id, $max_id );
					$worksheet->freezePaneByColumnAndRow( 2, 2 );
					// creating the Rewards worksheet
					$workbook->createSheet();
					$workbook->setActiveSheetIndex($worksheet_index++);
					$worksheet = $workbook->getActiveSheet();
					$worksheet->setTitle( 'Bundles' );
					$this->populateBundlesWorksheet( $worksheet, $language_id, $price_format, $box_format, $text_format, $min_id, $max_id );
					$worksheet->freezePaneByColumnAndRow( 2, 2 );
					// creating the Rewards worksheet
					$workbook->createSheet();
					$workbook->setActiveSheetIndex($worksheet_index++);
					...
					$this->populateBarcodesWorksheet( $worksheet, $box_format, $text_format, $min_id, $max_id );
					$worksheet->freezePaneByColumnAndRow( 2, 2 );
...
protected function uploadProductUnits(&$reader, $incremental, &$available_product_ids) {
    // Get worksheet, if not there return immediately
    $data = $reader->getSheetByName('Units');
    if ($data == null) {
        return;
    }

    // If incremental then find current product IDs else delete all old units
    if ($incremental) {
        $unlisted_product_ids = $available_product_ids;
    } else {
        $this->deleteProductUnits();
    }
    ...
    for ($i = 0; $i < $k; $i += 1) {
        ...
        $unit_id = trim($this->getCell($data, $i, $j++));
        if ($unit_id == "") {
            continue;
        }
        
        $unit_type = trim($this->getCell($data, $i, $j++));
        if (!in_array($unit_type, ['base', 'additional'])) {
            $unit_type = 'base';
        }
        
        $conversion_factor = $this->getCell($data, $i, $j++, '1.0000');
        ...
        if (($incremental) && ($product_id != $previous_product_id)) {
            $this->deleteProductUnit($product_id);
            if (isset($unlisted_product_ids[$product_id])) {
                unset($unlisted_product_ids[$product_id]);
            }
        }
        
        $this->storeProductUnitIntoDatabase($unit);
        $previous_product_id = $product_id;
    }

    if ($incremental) {
        $this->deleteUnlistedProductUnits($unlisted_product_ids);
    }
}

protected function getProductUnits($product_id = null) {
    $sql = "SELECT pu.*, CONCAT(u.desc_Ar,'-',u.desc_en) as unit_name 
            FROM `" . DB_PREFIX . "product_unit` pu 
            LEFT JOIN `" . DB_PREFIX . "unit` u ON (pu.unit_id = u.unit_id)";
    
    if ($product_id !== null) {
        $sql .= " WHERE pu.product_id = '" . (int)$product_id . "'";
    }
    
    $sql .= " ORDER BY pu.product_id, pu.unit_type DESC";
    
    $query = $this->db->query($sql);
    return $query->rows;
}

protected function storeProductUnitIntoDatabase(&$unit) {
    $product_id = $unit['product_id'];
    $unit_id = $unit['unit_id'];
    $unit_type = $unit['unit_type'];
    $conversion_factor = $unit['conversion_factor'];

    $sql = "INSERT INTO `" . DB_PREFIX . "product_unit` 
            (product_id, unit_id, unit_type, conversion_factor) 
            VALUES 
            ($product_id, $unit_id, '$unit_type', $conversion_factor)";
...
protected function deleteProductUnits() {
    $sql = "TRUNCATE TABLE `" . DB_PREFIX . "product_unit`";
    $this->db->query($sql);
}

protected function deleteProductUnit($product_id) {
    $sql = "DELETE FROM `" . DB_PREFIX . "product_unit` 
            WHERE product_id = '" . (int)$product_id . "'";
    $this->db->query($sql);
}

protected function deleteUnlistedProductUnits(&$unlisted_product_ids) {
    foreach ($unlisted_product_ids as $product_id) {
        $this->deleteProductUnit($product_id);
    }
}

protected function populateProductUnitsWorksheet(&$worksheet, &$box_format, &$text_format, $min_id = null, $max_id = null) {
    // Set the column widths
    $j = 1;
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('product_id') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('unit_id') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('unit_type') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('conversion_factor') + 1);
    ...
    $data[$j++] = 'unit_id';
    $data[$j++] = 'unit_type';
    $data[$j++] = 'conversion_factor';

    $worksheet->getRowDimension($i)->setRowHeight(30);
    $this->setCellRow($worksheet, $i, $data, $box_format);

    // The actual product units data
    $i += 1;
    $j = 1;
    
    $sql_where = '';
    if (isset($min_id) && isset($max_id)) {
        $sql_where = " WHERE product_id BETWEEN $min_id AND $max_id";
    }
    
    $product_units = $this->getProductUnits();
    foreach ($product_units as $row) {
        $worksheet->getRowDimension($i)->setRowHeight(13);
        $data = array();
        
        $data[$j++] = $row['product_id'];
        $data[$j++] = $row['unit_id'];
        $data[$j++] = $row['unit_type'];
        $data[$j++] = $row['conversion_factor'];
...
protected function storeInventoryIntoDatabase(&$inventory, &$old_product_inventory_ids) {
    ...
    $consignment_supplier_id = isset($inventory['consignment_supplier_id']) ? $inventory['consignment_supplier_id'] : 'NULL';
    $average_cost = $inventory['average_cost'];

    if (isset($old_product_inventory_ids[$product_id][$branch_id][$unit_id])) {
        $product_inventory_id = $old_product_inventory_ids[$product_id][$branch_id][$unit_id];
        $sql = "INSERT INTO `" . DB_PREFIX . "product_inventory` 
                (`product_inventory_id`, `product_id`, `branch_id`, `unit_id`, 
                 `quantity`, `quantity_available`, `is_consignment`, 
                 `consignment_supplier_id`, `average_cost`) VALUES 
                ($product_inventory_id, $product_id, $branch_id, $unit_id, 
                 $quantity, $quantity_available, $is_consignment, 
                 $consignment_supplier_id, $average_cost)";
        $this->db->query($sql);
        unset($old_product_inventory_ids[$product_id][$branch_id][$unit_id]);
    } else {
        $sql = "INSERT INTO `" . DB_PREFIX . "product_inventory` 
                (`product_id`, `branch_id`, `unit_id`, `quantity`, 
                 `quantity_available`, `is_consignment`, 
                 `consignment_supplier_id`, `average_cost`) VALUES 
                ($product_id, $branch_id, $unit_id, $quantity, 
                 $quantity_available, $is_consignment, 
                 $consignment_supplier_id, $average_cost)";
...
protected function deleteInventory($product_id) {
    $sql = "SELECT product_inventory_id, product_id, branch_id, unit_id 
            FROM `" . DB_PREFIX . "product_inventory` 
            WHERE product_id = '" . (int)$product_id . "'";
    $query = $this->db->query($sql);
    
    $old_product_inventory_ids = array();
    foreach ($query->rows as $row) {
        $product_inventory_id = $row['product_inventory_id'];
        $product_id = $row['product_id'];
        $branch_id = $row['branch_id'];
        $unit_id = $row['unit_id'];
        $old_product_inventory_ids[$product_id][$branch_id][$unit_id] = $product_inventory_id;
    }
    
    if ($old_product_inventory_ids) {
        $sql = "DELETE FROM `" . DB_PREFIX . "product_inventory` 
                WHERE product_id = '" . (int)$product_id . "'";
        $this->db->query($sql);
    }
    return $old_product_inventory_ids;
}

protected function deleteInventories() {
    $sql = "TRUNCATE TABLE `" . DB_PREFIX . "product_inventory`";
    $this->db->query($sql);
}

protected function deleteUnlistedInventories(&$unlisted_product_ids) {
    foreach ($unlisted_product_ids as $product_id) {
        $sql = "DELETE FROM `" . DB_PREFIX . "product_inventory` 
                WHERE product_id = '" . (int)$product_id . "'";
        $this->db->query($sql);
...
protected function getInventories($min_id = null, $max_id = null) {
    $sql = "SELECT DISTINCT pi.*, b.name as branch_name, CONCAT(u.desc_Ar,'-',u.desc_en) as unit_name,
            IFNULL(CONCAT(s.firstname,' ',s.lastname), '') as supplier_name  
            FROM `" . DB_PREFIX . "product_inventory` pi 
            LEFT JOIN `" . DB_PREFIX . "branch` b ON b.branch_id = pi.branch_id 
            LEFT JOIN `" . DB_PREFIX . "unit` u ON u.unit_id = pi.unit_id 
            LEFT JOIN `" . DB_PREFIX . "supplier` s ON s.supplier_id = pi.consignment_supplier_id ";
    ...
    $sql .= "ORDER BY pi.product_id, pi.branch_id, pi.unit_id";
    $result = $this->db->query($sql);
    return $result->rows;
}

protected function populateInventoryWorksheet(&$worksheet, &$price_format, &$box_format, &$text_format, $min_id = null, $max_id = null) {
    // Set the column widths
    $j = 1;
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('product_id') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('branch_name') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('unit_name') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('quantity'), 10) + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('quantity_available'), 10) + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('is_consignment') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('supplier_name') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('average_cost'), 10) + 1);
    ...
    $data[$j++] = 'quantity';
    $styles[$j] = &$price_format;
    $data[$j++] = 'quantity_available';
    $data[$j++] = 'is_consignment';
    $styles[$j] = &$text_format;
    $data[$j++] = 'supplier_name';
    $styles[$j] = &$price_format;
    $data[$j++] = 'average_cost';
    ...
    $i += 1;
    $j = 1;
    $inventories = $this->getInventories($min_id, $max_id);
    ...
    foreach ($inventories as $row) {
        ...
        $data[$j++] = $row['unit_name'];
        $data[$j++] = $row['quantity'];
        $data[$j++] = $row['quantity_available'];
        $data[$j++] = $row['is_consignment'];
        $data[$j++] = $row['supplier_name'];
        $data[$j++] = $row['average_cost'];
...
protected function uploadInventory(&$reader, $incremental, &$available_product_ids) {
    // Get worksheet, if not there return immediately
    $data = $reader->getSheetByName('Inventory');
    if ($data == null) {
        return;
    }

    // If incremental then find current product IDs else delete all old inventory
    if ($incremental) {
        $unlisted_product_ids = $available_product_ids;
    } else {
    ...
    for ($i = 0; $i < $k; $i += 1) {
        ...
        $branch_id = trim($this->getCell($data, $i, $j++));
        if ($branch_id == "") {
            continue;
        }
        ...
        $unit_id = trim($this->getCell($data, $i, $j++));
        $quantity = $this->getCell($data, $i, $j++, '0');
        $quantity_available = $this->getCell($data, $i, $j++, '0');
        $is_consignment = $this->getCell($data, $i, $j++, '0');
        $consignment_supplier_id = $this->getCell($data, $i, $j++, null);
        $average_cost = $this->getCell($data, $i, $j++, '0');
        ...
        if (($incremental) && ($product_id != $previous_product_id)) {
            $old_product_inventory_ids = $this->deleteInventory($product_id);
            if (isset($unlisted_product_ids[$product_id])) {
                unset($unlisted_product_ids[$product_id]);
            }
        }

        $this->storeInventoryIntoDatabase($inventory, $old_product_inventory_ids);
        $previous_product_id = $product_id;
    }

    if ($incremental) {
        $this->deleteUnlistedInventories($unlisted_product_ids);
    }
}

protected function storeBarcodeIntoDatabase(&$barcode, &$old_product_barcode_ids) {
    $product_id = $barcode['product_id'];
    $unit_id = $barcode['unit_id'];
    $barcode_value = $barcode['barcode'];
    $product_option_id = isset($barcode['product_option_id']) ? $barcode['product_option_id'] : 'NULL';
    $product_option_value_id = isset($barcode['product_option_value_id']) ? $barcode['product_option_value_id'] : 'NULL';

    if (isset($old_product_barcode_ids[$product_id][$unit_id][$barcode_value])) {
        $product_barcode_id = $old_product_barcode_ids[$product_id][$unit_id][$barcode_value];
        $sql = "INSERT INTO `" . DB_PREFIX . "product_barcode` 
                (`product_barcode_id`, `product_id`, `unit_id`, `barcode`, `product_option_id`, `product_option_value_id`) 
                VALUES ($product_barcode_id, $product_id, $unit_id, '$barcode_value', $product_option_id, $product_option_value_id)";
        $this->db->query($sql);
        unset($old_product_barcode_ids[$product_id][$unit_id][$barcode_value]);
    } else {
        $sql = "INSERT INTO `" . DB_PREFIX . "product_barcode` 
                (`product_id`, `unit_id`, `barcode`, `product_option_id`, `product_option_value_id`) 
                VALUES ($product_id, $unit_id, '$barcode_value', $product_option_id, $product_option_value_id)";
        $this->db->query($sql);
    }
}

protected function deleteBarcodes() {
    $sql = "TRUNCATE TABLE `" . DB_PREFIX . "product_barcode`";
    $this->db->query($sql);
}

protected function deleteBarcode($product_id) {
    $sql = "SELECT product_barcode_id, product_id, unit_id, barcode 
            FROM `" . DB_PREFIX . "product_barcode` 
            WHERE product_id = '" . (int)$product_id . "'";
    $query = $this->db->query($sql);
    
    $old_product_barcode_ids = array();
    foreach ($query->rows as $row) {
        $product_barcode_id = $row['product_barcode_id'];
        $product_id = $row['product_id'];
        $unit_id = $row['unit_id'];
        $barcode = $row['barcode'];
        $old_product_barcode_ids[$product_id][$unit_id][$barcode] = $product_barcode_id;
    }
    
    if ($old_product_barcode_ids) {
        $sql = "DELETE FROM `" . DB_PREFIX . "product_barcode` 
                WHERE product_id = '" . (int)$product_id . "'";
        $this->db->query($sql);
    }
    
    return $old_product_barcode_ids;
}

protected function deleteUnlistedBarcodes(&$unlisted_product_ids) {
    foreach ($unlisted_product_ids as $product_id) {
        $sql = "DELETE FROM `" . DB_PREFIX . "product_barcode` 
                WHERE product_id = '" . (int)$product_id . "'";
        $this->db->query($sql);
...
protected function uploadBarcodes(&$reader, $incremental, &$available_product_ids) {
    // Get worksheet, if not there return immediately
    $data = $reader->getSheetByName('Barcodes');
    if ($data == null) {
        return;
    }

    // If incremental then find current product IDs else delete all old barcodes
    if ($incremental) {
        $unlisted_product_ids = $available_product_ids;
    } else {
        $this->deleteBarcodes();
    ...
    for ($i = 0; $i < $k; $i += 1) {
        ...
        $barcode_value = trim($this->getCell($data, $i, $j++));
        $product_option_id = trim($this->getCell($data, $i, $j++));
        $product_option_value_id = trim($this->getCell($data, $i, $j++));
        
        $barcode = array();
        $barcode['product_id'] = $product_id;
        $barcode['unit_id'] = $unit_id;
        $barcode['barcode'] = $barcode_value;
        if ($product_option_id) {
            $barcode['product_option_id'] = $product_option_id;
        }
        if ($product_option_value_id) {
            $barcode['product_option_value_id'] = $product_option_value_id;
        }
        
        if (($incremental) && ($product_id != $previous_product_id)) {
            $old_product_barcode_ids = $this->deleteBarcode($product_id);
            if (isset($unlisted_product_ids[$product_id])) {
                unset($unlisted_product_ids[$product_id]);
            }
        }
        
        $this->storeBarcodeIntoDatabase($barcode, $old_product_barcode_ids);
        $previous_product_id = $product_id;
    }
    
    if ($incremental) {
        $this->deleteUnlistedBarcodes($unlisted_product_ids);
    }
}

protected function getBarcodes($min_id = null, $max_id = null) {
    $sql = "SELECT DISTINCT pb.*, p.product_id 
            FROM `" . DB_PREFIX . "product_barcode` pb 
            LEFT JOIN `" . DB_PREFIX . "product` p ON p.product_id = pb.product_id ";
    
    if ($this->posted_categories) {
        $sql .= "LEFT JOIN `" . DB_PREFIX . "product_to_category` pc ON pc.product_id = pb.product_id ";
    }
    
    if (isset($min_id) && isset($max_id)) {
        $sql .= "WHERE pb.product_id BETWEEN $min_id AND $max_id ";
        if ($this->posted_categories) {
            $sql .= "AND pc.category_id IN " . $this->posted_categories . " ";
        }
    } else if ($this->posted_categories) {
    ...
    $sql .= "ORDER BY pb.product_id, pb.unit_id, pb.barcode";
    $result = $this->db->query($sql);
    return $result->rows;
}

protected function populateBarcodesWorksheet(&$worksheet, &$box_format, &$text_format, $min_id = null, $max_id = null) {
    // Set the column widths
    $j = 1;
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('product_id') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('unit_id') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('barcode') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('product_option_id') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('product_option_value_id') + 1);
    ...
    $styles[$j] = &$text_format;
    $data[$j++] = 'barcode';
    $data[$j++] = 'product_option_id';
    $data[$j++] = 'product_option_value_id';
    
    $worksheet->getRowDimension($i)->setRowHeight(30);
    $this->setCellRow($worksheet, $i, $data, $box_format);
    
    // The actual product barcodes data
    $i += 1;
    $j = 1;
    $barcodes = $this->getBarcodes($min_id, $max_id);
    ...
    foreach ($barcodes as $row) {
        ...
        $data[$j++] = $row['unit_id'];
        $data[$j++] = $row['barcode'];
        $data[$j++] = $row['product_option_id'];
        $data[$j++] = $row['product_option_value_id'];
...
protected function getProductPricing($language_id, $min_id = null, $max_id = null) {
    $sql = "SELECT DISTINCT pp.*, CONCAT(u.desc_Ar,'-',u.desc_en) as unit_name 
            FROM `" . DB_PREFIX . "product_pricing` pp 
            LEFT JOIN `" . DB_PREFIX . "unit` u ON u.unit_id = pp.unit_id ";
    
    if ($this->posted_categories) {
        $sql .= "LEFT JOIN `" . DB_PREFIX . "product_to_category` pc ON pc.product_id = pp.product_id ";
    }
    
    if ($this->posted_manufacturers) {
        $sql .= "LEFT JOIN `" . DB_PREFIX . "product` p ON p.product_id = pp.product_id ";
    }
    
    if (isset($min_id) && isset($max_id)) {
        $sql .= "WHERE pp.product_id BETWEEN $min_id AND $max_id ";
        if ($this->posted_categories) {
            $sql .= "AND pc.category_id IN " . $this->posted_categories . " ";
        }
    } else if ($this->posted_categories) {
    ...
    $sql .= "ORDER BY pp.product_id, pp.unit_id";
    $result = $this->db->query($sql);
    return $result->rows;
}

protected function storePricingIntoDatabase(&$pricing, &$old_product_pricing_ids) {
    $product_id = $pricing['product_id'];
    $unit_id = $pricing['unit_id'];
    $base_price = $pricing['base_price'];
    $special_price = isset($pricing['special_price']) ? $pricing['special_price'] : 'NULL';
    $wholesale_price = isset($pricing['wholesale_price']) ? $pricing['wholesale_price'] : 'NULL';
    $half_wholesale_price = isset($pricing['half_wholesale_price']) ? $pricing['half_wholesale_price'] : 'NULL';
    $custom_price = isset($pricing['custom_price']) ? $pricing['custom_price'] : 'NULL';

    if (isset($old_product_pricing_ids[$product_id][$unit_id])) {
        $product_pricing_id = $old_product_pricing_ids[$product_id][$unit_id];
        $sql = "INSERT INTO `" . DB_PREFIX . "product_pricing` 
                (`product_pricing_id`, `product_id`, `unit_id`, `base_price`, 
                 `special_price`, `wholesale_price`, `half_wholesale_price`, `custom_price`) 
                VALUES 
                ($product_pricing_id, $product_id, $unit_id, $base_price, 
                 $special_price, $wholesale_price, $half_wholesale_price, $custom_price)";
        $this->db->query($sql);
        unset($old_product_pricing_ids[$product_id][$unit_id]);
    } else {
        $sql = "INSERT INTO `" . DB_PREFIX . "product_pricing` 
                (`product_id`, `unit_id`, `base_price`, 
                 `special_price`, `wholesale_price`, `half_wholesale_price`, `custom_price`) 
                VALUES 
                ($product_id, $unit_id, $base_price, 
                 $special_price, $wholesale_price, $half_wholesale_price, $custom_price)";
...
protected function deletePricing($product_id) {
    $sql = "SELECT product_pricing_id, product_id, unit_id 
            FROM `" . DB_PREFIX . "product_pricing` 
            WHERE product_id = '" . (int)$product_id . "'";
    $query = $this->db->query($sql);
    
    $old_product_pricing_ids = array();
    foreach ($query->rows as $row) {
        $product_pricing_id = $row['product_pricing_id'];
        $product_id = $row['product_id'];
        $unit_id = $row['unit_id'];
        $old_product_pricing_ids[$product_id][$unit_id] = $product_pricing_id;
    }
    
    if ($old_product_pricing_ids) {
        $sql = "DELETE FROM `" . DB_PREFIX . "product_pricing` 
                WHERE product_id = '" . (int)$product_id . "'";
        $this->db->query($sql);
    }
    return $old_product_pricing_ids;
}

protected function deleteUnlistedPricing(&$unlisted_product_ids) {
    foreach ($unlisted_product_ids as $product_id) {
        $sql = "DELETE FROM `" . DB_PREFIX . "product_pricing` 
                WHERE product_id = '" . (int)$product_id . "'";
        $this->db->query($sql);
...
protected function uploadPricing(&$reader, $incremental, &$available_product_ids) {
    // Get worksheet, if not there return immediately
    $data = $reader->getSheetByName('Pricing');
    if ($data == null) {
        return;
    }

    // If incremental then find current product IDs else delete all old pricing
    if ($incremental) {
        $unlisted_product_ids = $available_product_ids;
    } else {
        $this->deletePricing();
    ...
    for ($i = 0; $i < $k; $i += 1) {
        ...
        $base_price = $this->getCell($data, $i, $j++, '0.00');
        $special_price = $this->getCell($data, $i, $j++);
        $wholesale_price = $this->getCell($data, $i, $j++);
        $half_wholesale_price = $this->getCell($data, $i, $j++);
        $custom_price = $this->getCell($data, $i, $j++);
        ...
        if (($incremental) && ($product_id != $previous_product_id)) {
            $old_product_pricing_ids = $this->deletePricing($product_id);
            if (isset($unlisted_product_ids[$product_id])) {
                unset($unlisted_product_ids[$product_id]);
            }
        }

        $this->storePricingIntoDatabase($pricing, $old_product_pricing_ids);
        $previous_product_id = $product_id;
    }

    if ($incremental) {
        $this->deleteUnlistedPricing($unlisted_product_ids);
    }
}

protected function populatePricingWorksheet(&$worksheet, $language_id, &$price_format, &$box_format, &$text_format, $min_id = null, $max_id = null) {
    // Set the column widths
    $j = 1;
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('product_id') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('unit_id') + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('base_price'), 10) + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('special_price'), 10) + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('wholesale_price'), 10) + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('half_wholesale_price'), 10) + 1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('custom_price'), 10) + 1);
    ...
    $styles[$j] = &$price_format;
    $data[$j++] = 'wholesale_price';
    $styles[$j] = &$price_format;
    $data[$j++] = 'half_wholesale_price';
    $styles[$j] = &$price_format;
    $data[$j++] = 'custom_price';
    ...
    // The actual product pricing data
    $i += 1;
    $j = 1;
    $pricing_data = $this->getProductPricing($language_id, $min_id, $max_id);
    ...
    foreach ($pricing_data as $row) {
        ...
        $data[$j++] = $row['product_id'];
        $data[$j++] = $row['unit_id'];
        $data[$j++] = $row['base_price'];
        $data[$j++] = $row['special_price'];
        $data[$j++] = $row['wholesale_price'];
        $data[$j++] = $row['half_wholesale_price'];
        $data[$j++] = $row['custom_price'];
...
protected function getQuantityDiscounts($language_id, $min_id=null, $max_id=null) {
    $sql = "SELECT DISTINCT pqd.*, CONCAT(u.desc_Ar,'-',u.desc_en) as unit_name 
            FROM `" . DB_PREFIX . "product_quantity_discounts` pqd 
            LEFT JOIN `" . DB_PREFIX . "unit` u ON u.unit_id = pqd.unit_id ";
    
    if ($this->posted_categories) {
        $sql .= "LEFT JOIN `" . DB_PREFIX . "product_to_category` pc ON pc.product_id=pqd.product_id ";
    }
    if ($this->posted_manufacturers) {
        $sql .= "LEFT JOIN `" . DB_PREFIX . "product` p ON p.product_id=pqd.product_id ";
    }
    
    if (isset($min_id) && isset($max_id)) {
        $sql .= "WHERE pqd.product_id BETWEEN $min_id AND $max_id ";
        if ($this->posted_categories) {
            $sql .= "AND pc.category_id IN " . $this->posted_categories . " ";
        }
    } else if ($this->posted_categories) {
    ...
    $sql .= "ORDER BY pqd.product_id, pqd.buy_quantity";
    $result = $this->db->query($sql);
    return $result->rows;
}

protected function storeQuantityDiscountIntoDatabase(&$discount, &$old_quantity_discount_ids) {
    ...
    $unit_id = isset($discount['unit_id']) ? $discount['unit_id'] : 37; // default unit_id
    $date_start = $discount['date_start'];
    $date_end = $discount['date_end'];
    $status = isset($discount['status']) ? $discount['status'] : 1;
    $notes = isset($discount['notes']) ? $discount['notes'] : '';

    // Check if discount already exists
    if (isset($old_quantity_discount_ids[$product_id])) {
        $discount_id = $old_quantity_discount_ids[$product_id];
        $sql = "INSERT INTO `" . DB_PREFIX . "product_quantity_discounts` 
                (`discount_id`, `product_id`, `name`, `type`, `buy_quantity`, 
                 `get_quantity`, `discount_type`, `discount_value`, `status`, 
                 `unit_id`, `date_start`, `date_end`, `notes`) VALUES 
                ($discount_id, $product_id, '$name', '$type', $buy_quantity, 
                 $get_quantity, '$discount_type', $discount_value, $status, 
                 $unit_id, '$date_start', '$date_end', '$notes')";
        $this->db->query($sql);
        unset($old_quantity_discount_ids[$product_id]);
    } else {
        $sql = "INSERT INTO `" . DB_PREFIX . "product_quantity_discounts` 
                (`product_id`, `name`, `type`, `buy_quantity`, 
                 `get_quantity`, `discount_type`, `discount_value`, `status`, 
                 `unit_id`, `date_start`, `date_end`, `notes`) VALUES 
                ($product_id, '$name', '$type', $buy_quantity, 
                 $get_quantity, '$discount_type', $discount_value, $status, 
...
protected function deleteQuantityDiscounts() {
    $sql = "TRUNCATE TABLE `" . DB_PREFIX . "product_quantity_discounts`";
    $this->db->query($sql);
}

protected function deleteQuantityDiscount($product_id) {
    $sql = "SELECT discount_id, product_id FROM `" . DB_PREFIX . "product_quantity_discounts` 
            WHERE product_id='" . (int)$product_id . "'";
    $query = $this->db->query($sql);
    $old_quantity_discount_ids = array();
    foreach ($query->rows as $row) {
        $old_quantity_discount_ids[$row['product_id']] = $row['discount_id'];
    }
    if ($old_quantity_discount_ids) {
        $sql = "DELETE FROM `" . DB_PREFIX . "product_quantity_discounts` 
                WHERE product_id='" . (int)$product_id . "'";
        $this->db->query($sql);
    }
    return $old_quantity_discount_ids;
}

protected function deleteUnlistedQuantityDiscounts(&$unlisted_product_ids) {
    foreach ($unlisted_product_ids as $product_id) {
        $sql = "DELETE FROM `" . DB_PREFIX . "product_quantity_discounts` 
                WHERE product_id='" . (int)$product_id . "'";
        $this->db->query($sql);
...
protected function populateQuantityDiscountsWorksheet(&$worksheet, $language_id, &$price_format, &$box_format, &$text_format, $min_id=null, $max_id=null) {
    // Set column widths
    $j = 1;
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('product_id')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('name')+20);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('type')+5);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('buy_quantity')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('get_quantity')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('discount_type')+5);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('discount_value')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('unit_id')+1);
    ...
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('notes')+20);
    ...
    $i += 1;
    $j = 1;
    $discounts = $this->getQuantityDiscounts($language_id, $min_id, $max_id);
    foreach ($discounts as $row) {
        ...
        $data[$j++] = $row['name'];
        $data[$j++] = $row['type'];
        $data[$j++] = $row['buy_quantity'];
        $data[$j++] = $row['get_quantity'];
        $data[$j++] = $row['discount_type'];
        $data[$j++] = $row['discount_value'];
        $data[$j++] = $row['unit_id'];
...
protected function uploadQuantityDiscounts(&$reader, $incremental, &$available_product_ids) {
    // Get worksheet, if not there return immediately
    ...
    // If incremental then find current product IDs else delete all old quantity discounts
    if ($incremental) {
        $unlisted_product_ids = $available_product_ids;
    } else {
        $this->deleteQuantityDiscounts();
    }
    ...
    for ($i = 0; $i < $k; $i += 1) {
        ...
        $type = trim($this->getCell($data, $i, $j++));
        $buy_quantity = $this->getCell($data, $i, $j++, '0');
        $get_quantity = $this->getCell($data, $i, $j++, '0');
        $discount_type = trim($this->getCell($data, $i, $j++));
        $discount_value = $this->getCell($data, $i, $j++, '0');
        $unit_id = $this->getCell($data, $i, $j++, '37');
        $date_start = $this->getCell($data, $i, $j++, '0000-00-00');
        $date_end = $this->getCell($data, $i, $j++, '0000-00-00');
        $status = $this->getCell($data, $i, $j++, '1');
        $notes = trim($this->getCell($data, $i, $j++));
        ...
        if (($incremental) && ($product_id != $previous_product_id)) {
            $old_quantity_discount_ids = $this->deleteQuantityDiscount($product_id);
            if (isset($unlisted_product_ids[$product_id])) {
                unset($unlisted_product_ids[$product_id]);
            }
        }
        
        $this->storeQuantityDiscountIntoDatabase($discount, $old_quantity_discount_ids);
        $previous_product_id = $product_id;
    }
    
    if ($incremental) {
        $this->deleteUnlistedQuantityDiscounts($unlisted_product_ids);
    }
}

protected function storeRecommendationIntoDatabase(&$recommendation, &$old_recommendation_ids) {
    $product_id = $recommendation['product_id'];
    $related_product_id = $recommendation['related_product_id'];
    $unit_id = $recommendation['unit_id'];
    $customer_group_id = !empty($recommendation['customer_group_id']) ? $recommendation['customer_group_id'] : 'NULL';
    $discount_type = !empty($recommendation['discount_type']) ? "'".$recommendation['discount_type']."'" : 'NULL';
    $discount_value = !empty($recommendation['discount_value']) ? $recommendation['discount_value'] : '0.0000';
    $type = $recommendation['type'];
    $priority = isset($recommendation['priority']) ? $recommendation['priority'] : 0;

    if (isset($old_recommendation_ids[$product_id][$related_product_id])) {
        $recommendation_id = $old_recommendation_ids[$product_id][$related_product_id];
        $sql = "INSERT INTO `".DB_PREFIX."product_recommendation` 
                (`recommendation_id`, `product_id`, `related_product_id`, `unit_id`, 
                 `customer_group_id`, `discount_type`, `discount_value`, `type`, `priority`) 
                VALUES 
                ($recommendation_id, $product_id, $related_product_id, $unit_id, 
                 $customer_group_id, $discount_type, $discount_value, '$type', $priority)";
        $this->db->query($sql);
        unset($old_recommendation_ids[$product_id][$related_product_id]);
    } else {
        $sql = "INSERT INTO `".DB_PREFIX."product_recommendation` 
                (`product_id`, `related_product_id`, `unit_id`, 
                 `customer_group_id`, `discount_type`, `discount_value`, `type`, `priority`) 
                VALUES 
                ($product_id, $related_product_id, $unit_id, 
                 $customer_group_id, $discount_type, $discount_value, '$type', $priority)";
...
protected function deleteRecommendations() {
    $sql = "TRUNCATE TABLE `".DB_PREFIX."product_recommendation`";
    $this->db->query($sql);
}

protected function deleteRecommendation($product_id) {
    $sql = "SELECT recommendation_id, product_id, related_product_id 
            FROM `".DB_PREFIX."product_recommendation` 
            WHERE product_id = '".(int)$product_id."'";
    $query = $this->db->query($sql);
    
    $old_recommendation_ids = array();
    foreach ($query->rows as $row) {
        $recommendation_id = $row['recommendation_id'];
        $product_id = $row['product_id'];
        $related_product_id = $row['related_product_id'];
        $old_recommendation_ids[$product_id][$related_product_id] = $recommendation_id;
    }
    
    if ($old_recommendation_ids) {
        $sql = "DELETE FROM `".DB_PREFIX."product_recommendation` 
                WHERE product_id = '".(int)$product_id."'";
        $this->db->query($sql);
    }
    return $old_recommendation_ids;
}

protected function deleteUnlistedRecommendations(&$unlisted_product_ids) {
    foreach ($unlisted_product_ids as $product_id) {
        $sql = "DELETE FROM `".DB_PREFIX."product_recommendation` 
                WHERE product_id = '".(int)$product_id."'";
        $this->db->query($sql);
...
protected function getRecommendations($language_id, $min_id=null, $max_id=null) {
    $sql = "SELECT DISTINCT pr.*, 
            p.model as related_product_model,
            pd.name as related_product_name,
            CONCAT(u.desc_Ar,'-',u.desc_en) as unit_name,
            cgd.name as customer_group 
            FROM `".DB_PREFIX."product_recommendation` pr 
            LEFT JOIN `".DB_PREFIX."product` p ON p.product_id = pr.related_product_id 
            LEFT JOIN `".DB_PREFIX."product_description` pd ON pd.product_id = pr.related_product_id 
                AND pd.language_id = '".(int)$language_id."'
            LEFT JOIN `".DB_PREFIX."unit` u ON u.unit_id = pr.unit_id 
            LEFT JOIN `".DB_PREFIX."customer_group_description` cgd 
                ON cgd.customer_group_id = pr.customer_group_id 
                AND cgd.language_id = '".(int)$language_id."'";
    ...
    if ($this->posted_categories) {
        $sql .= (strpos($sql, " WHERE ", 0) === false) ? " WHERE " : " AND ";
        $sql .= "pr.product_id IN (SELECT product_id FROM `".DB_PREFIX."product_to_category` 
                WHERE category_id IN ".$this->posted_categories.")";
    }
    
    if ($this->posted_manufacturers) {
        $sql .= (strpos($sql, " WHERE ", 0) === false) ? " WHERE " : " AND ";
        $sql .= "pr.product_id IN (SELECT product_id FROM `".DB_PREFIX."product` 
                WHERE manufacturer_id IN ".$this->posted_manufacturers.")";
    }
    
    $sql .= " ORDER BY pr.product_id, pr.type, pr.priority";
    $result = $this->db->query($sql);
    return $result->rows;
}

protected function populateRecommendationsWorksheet(&$worksheet, $language_id, &$price_format, &$box_format, &$text_format, $min_id=null, $max_id=null) {
    // Set the column widths
    $j = 1;
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('product_id')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('related_product_id')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('related_product_name')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('unit_id')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('customer_group')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('discount_type')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('discount_value')+1);
    ...
    $data[$j++] = 'product_id';
    $data[$j++] = 'related_product_id';
    $styles[$j] = &$text_format;
    $data[$j++] = 'related_product_name';
    $data[$j++] = 'unit_id';
    $styles[$j] = &$text_format;
    $data[$j++] = 'customer_group';
    $data[$j++] = 'discount_type';
    ...
    // The actual recommendations data
    $i += 1;
    $j = 1;
    $recommendations = $this->getRecommendations($language_id, $min_id, $max_id);
    
    foreach ($recommendations as $row) {
        $worksheet->getRowDimension($i)->setRowHeight(13);
        $data = array();
        $data[$j++] = $row['product_id'];
        $data[$j++] = $row['related_product_id'];
        $data[$j++] = $row['related_product_name'];
        $data[$j++] = $row['unit_id'];
        $data[$j++] = $row['customer_group'];
        $data[$j++] = $row['discount_type'];
        $data[$j++] = $row['discount_value'];
...
protected function uploadRecommendations(&$reader, $incremental, &$available_product_ids) {
    // Get worksheet, if not there return immediately
    $data = $reader->getSheetByName('Recommendations');
    if ($data == null) {
        return;
    }

    // If incremental then find current product IDs else delete all old recommendations
    if ($incremental) {
        $unlisted_product_ids = $available_product_ids;
    } else {
    ...
    for ($i = 0; $i < $k; $i += 1) {
        ...
        $related_product_id = trim($this->getCell($data, $i, $j++));
        ...
        $customer_group = trim($this->getCell($data, $i, $j++));
        $discount_type = trim($this->getCell($data, $i, $j++));
        $discount_value = $this->getCell($data, $i, $j++, '0.0000');
        $type = trim($this->getCell($data, $i, $j++));
        ...
        if (($incremental) && ($product_id != $previous_product_id)) {
            $old_recommendation_ids = $this->deleteRecommendation($product_id);
            if (isset($unlisted_product_ids[$product_id])) {
                unset($unlisted_product_ids[$product_id]);
            }
        }

        $this->storeRecommendationIntoDatabase($recommendation, $old_recommendation_ids);
        $previous_product_id = $product_id;
    }

    if ($incremental) {
        $this->deleteUnlistedRecommendations($unlisted_product_ids);
    }
}


protected function storeBundleIntoDatabase(&$bundle, &$old_bundle_ids) {
    $product_id = $bundle['product_id'];
    $name = $this->db->escape($bundle['name']);
    $discount_type = $bundle['discount_type'];
    $discount_value = !empty($bundle['discount_value']) ? $bundle['discount_value'] : '0.0000';
    $status = isset($bundle['status']) ? (int)$bundle['status'] : 1;

    if (isset($old_bundle_ids[$product_id])) {
        $bundle_id = $old_bundle_ids[$product_id];
        $sql = "INSERT INTO `".DB_PREFIX."product_bundle` 
                (`bundle_id`, `product_id`, `name`, `discount_type`, 
                 `discount_value`, `status`) 
                VALUES 
                ($bundle_id, $product_id, '$name', '$discount_type', 
                 $discount_value, $status)";
        $this->db->query($sql);
        unset($old_bundle_ids[$product_id]);
    } else {
        $sql = "INSERT INTO `".DB_PREFIX."product_bundle` 
                (`product_id`, `name`, `discount_type`, 
                 `discount_value`, `status`) 
                VALUES 
                ($product_id, '$name', '$discount_type', 
                 $discount_value, $status)";
    ...
    if (!empty($bundle['items'])) {
        foreach ($bundle['items'] as $item) {
            $this->storeBundleItemIntoDatabase($bundle_id, $item);
        }
    }
...
protected function storeBundleItemIntoDatabase($bundle_id, &$item) {
    $product_id = $item['product_id'];
    $quantity = !empty($item['quantity']) ? (int)$item['quantity'] : 1;
    $unit_id = $item['unit_id'];
    $is_free = isset($item['is_free']) ? (int)$item['is_free'] : 0;
    
    $sql = "INSERT INTO `".DB_PREFIX."product_bundle_item` 
            (`bundle_id`, `product_id`, `quantity`, `unit_id`, `is_free`) 
            VALUES 
            ($bundle_id, $product_id, $quantity, $unit_id, $is_free)";
    $this->db->query($sql);
}
...
protected function deleteBundles() {
    $this->db->query("TRUNCATE TABLE `".DB_PREFIX."product_bundle_item`");
    $this->db->query("TRUNCATE TABLE `".DB_PREFIX."product_bundle`");
}

protected function deleteBundle($product_id) {
    // Get existing bundle IDs first
    $sql = "SELECT bundle_id, product_id FROM `".DB_PREFIX."product_bundle` 
            WHERE product_id = '".(int)$product_id."'";
    $query = $this->db->query($sql);
    
    $old_bundle_ids = array();
    foreach ($query->rows as $row) {
        $old_bundle_ids[$row['product_id']] = $row['bundle_id'];
        // Delete bundle items
        $this->db->query("DELETE FROM `".DB_PREFIX."product_bundle_item` 
                         WHERE bundle_id = '".(int)$row['bundle_id']."'");
    }
    
    // Delete bundle
    if ($old_bundle_ids) {
        $this->db->query("DELETE FROM `".DB_PREFIX."product_bundle` 
                         WHERE product_id = '".(int)$product_id."'");
    }
    
    return $old_bundle_ids;
}

protected function deleteUnlistedBundles(&$unlisted_product_ids) {
    foreach ($unlisted_product_ids as $product_id) {
        $sql = "SELECT bundle_id FROM `".DB_PREFIX."product_bundle` 
                WHERE product_id = '".(int)$product_id."'";
        $query = $this->db->query($sql);
...
protected function getBundles($language_id, $min_id=null, $max_id=null) {
    $sql = "SELECT DISTINCT pb.*, 
            pd.name as product_name 
            FROM `".DB_PREFIX."product_bundle` pb 
    ...
    if ($this->posted_categories) {
        $sql .= (strpos($sql, " WHERE ", 0) === false) ? " WHERE " : " AND ";
        $sql .= "pb.product_id IN (SELECT product_id FROM `".DB_PREFIX."product_to_category` 
                WHERE category_id IN ".$this->posted_categories.")";
    }
    
    $sql .= " ORDER BY pb.product_id";
    $query = $this->db->query($sql);
    $bundles = $query->rows;
    
    // Get bundle items for each bundle
    foreach ($bundles as &$bundle) {
        $bundle['items'] = $this->getBundleItems($bundle['bundle_id'], $language_id);
    }
    
    return $bundles;
}

protected function getBundleItems($bundle_id, $language_id) {
    $sql = "SELECT bi.*, 
            pd.name as product_name,
            CONCAT(u.desc_Ar,'-',u.desc_en) as unit_name 
            FROM `".DB_PREFIX."product_bundle_item` bi 
            LEFT JOIN `".DB_PREFIX."product_description` pd 
                ON pd.product_id = bi.product_id 
                AND pd.language_id = '".(int)$language_id."' 
            LEFT JOIN `".DB_PREFIX."unit` u 
                ON u.unit_id = bi.unit_id 
            WHERE bi.bundle_id = '".(int)$bundle_id."'";
    
    $query = $this->db->query($sql);
    return $query->rows;
}

protected function populateBundlesWorksheet(&$worksheet, $language_id, &$price_format, &$box_format, &$text_format, $min_id=null, $max_id=null) {
    // Set the column widths
    $j = 1;
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('product_id')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('bundle_name')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('discount_type')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('discount_value')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('status')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('item_product_id')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('item_quantity')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('item_unit_id')+1);
    $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('is_free')+1);
    ...
    $data[$j++] = 'discount_value';
    $data[$j++] = 'status';
    $data[$j++] = 'item_product_id';
    $data[$j++] = 'item_quantity';
    $data[$j++] = 'item_unit_id';
    $data[$j++] = 'is_free';
    ...
    $i += 1;
    $j = 1;
    $bundles = $this->getBundles($language_id, $min_id, $max_id);
    
    foreach ($bundles as $bundle) {
        if (!empty($bundle['items'])) {
            foreach ($bundle['items'] as $item) {
                $data = array();
                $data[$j++] = $bundle['product_id'];
                $data[$j++] = $bundle['name'];
                $data[$j++] = $bundle['discount_type'];
                $data[$j++] = $bundle['discount_value'];
                $data[$j++] = $bundle['status'];
                $data[$j++] = $item['product_id'];
                $data[$j++] = $item['quantity'];
                $data[$j++] = $item['unit_id'];
                $data[$j++] = $item['is_free'];
...
protected function uploadBundles(&$reader, $incremental, &$available_product_ids) {
    // Get worksheet, if not there return immediately
    $data = $reader->getSheetByName('Bundles');
    if ($data == null) {
        return;
    }

    // If incremental then find current product IDs else delete all old bundles
    if ($incremental) {
        $unlisted_product_ids = $available_product_ids;
    } else {
        $this->deleteBundles();
    ...
    for ($i = 0; $i < $k; $i += 1) {
        ...
        $discount_value = $this->getCell($data, $i, $j++, '0.0000');
        $status = $this->getCell($data, $i, $j++, '1');
        $item_product_id = trim($this->getCell($data, $i, $j++));
        $item_quantity = $this->getCell($data, $i, $j++, '1');
        $item_unit_id = trim($this->getCell($data, $i, $j++));
        $is_free = $this->getCell($data, $i, $j++, '0');

        if ($product_id != $previous_product_id) {
            // Save previous bundle if exists
            if (!empty($current_bundle)) {
                if (($incremental) && ($previous_product_id != 0)) {
                    $old_bundle_ids = $this->deleteBundle($previous_product_id);
                    if (isset($unlisted_product_ids[$previous_product_id])) {
                        unset($unlisted_product_ids[$previous_product_id]);
                    }
                }
                $this->storeBundleIntoDatabase($current_bundle, $old_bundle_ids);
            }
        ...
        // Add item to current bundle if exists
        if ($item_product_id) {
            $current_bundle['items'][] = array(
    ...
    if ($incremental) {
        $this->deleteUnlistedBundles($unlisted_product_ids);
    }
}	
