...
public function getCustomerByEmailOrPhone($identifier) {
    // التحقق مما إذا كان المدخل هو بريد إلكتروني أو رقم هاتف
    if (filter_var($identifier, FILTER_VALIDATE_EMAIL)) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($identifier)) . "'");
    } else {
        // افتراض أن رقم الهاتف يخزن بدون كود الدولة وأنه يتم تخزين آخر 10 أرقام فقط
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE RIGHT(telephone, 10) = '" . $this->db->escape(substr($identifier, -10)) . "'");
    }
